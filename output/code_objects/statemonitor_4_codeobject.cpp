...
const size_t _numnot_refractory = 400;
    ...
    _dynamic_array_statemonitor_4_phi.resize(_new_size, _num_indices);
    ...
    for (int _i = 0; _i < (int)_num_indices; _i++)
    {
        ...
        const double _source_V = _ptr_array_neurongroup_8_V[_idx];
        const double _source_phi = _ptr_array_neurongroup_8_phi[_idx];
        const double _to_record_V = _source_V;
        const double _to_record_phi = _source_phi;


        _dynamic_array_statemonitor_4_phi(_new_size-1, _i) = _to_record_phi;
